...
export function Post({ post, state }: { post: Post; state?: string }) {
  ...
  return (
    <Card className="h-fit p-[3vw] flex items-center justify-center gap-[1.8vh] shadow-xl break-words whitespace-normal">
      <div className="flex items-center gap-[2vw]">
        ...
        />
        <CardTitle className="text-[2rem] capitalize">
      ...
      {post.imageUrl && (
        <img src={post.imageUrl as string} className="w-[30vw]" />
      )}
      {state != "profile" && (
        <button
          ...
          onClick={async () => {
            await navigator.clipboard.writeText(
              post.author.discordUrl as string
            );
            setCopied(true);
          }}
        >
